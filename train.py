...
def parse_args():
    ...
    parser.add_argument('--wandb_run_name', type=str, default='2-Stage_U++_resnet', help='WandB Run name 설정')
...
def main():
    ...
    # 손실 함수 및 옵티마이저 설정
    unet1 = smp.UnetPlusPlus(encoder_name="resnet34", encoder_weights="imagenet", in_channels=1, classes=1)  # 흑백 -> 마스크 복원
    unet2 = smp.UnetPlusPlus(encoder_name="resnet34", encoder_weights="imagenet", in_channels=1, classes=3)  # 흑백+복원 -> 색상 복원
