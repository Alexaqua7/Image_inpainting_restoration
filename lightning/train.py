...
def parse_args():
    ...
    parser.add_argument('--wandb_run_name', type=str, default='UNet++_HRNet_w64 + ResNet50', help='WandB Run name 설정')
...
def main():
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=True, persistent_workers=True,collate_fn=CollateFn(mode='train'))
    valid_dataloader = DataLoader(valid_dataset, batch_size=args.batch_size*2, shuffle=False, num_workers=8, pin_memory=True, persistent_workers=True,collate_fn=CollateFn(mode='valid'))
    ...
    trainer = L.Trainer(max_epochs=args.num_epoch, precision='16-mixed', callbacks=[checkpoint_callback, earlystopping_callback], detect_anomaly=False)
