...
def parse_args():
    ...
    parser.add_argument('--num_epoch', type=int, default=30, help='Epoch 수 설정')
    ...
    parser.add_argument('--wandb_run_name', type=str, default='unetpp_maxvit_base_tf_512 + EfficientNetb7 (New Curriculum)', help='WandB Run name 설정')
...
def main():
    ...
    train_fold_df, valid_fold_df = stratified_split_dataset(args.seed, args.n_split, train_df, args.train_data_dir, args.val_data_dir, args.min_polygon_bbox_size, args.max_polygon_bbox_size)
    # Wandb initalize
    wandb.init(
    ...
    # transform = A.Compose([
    # A.GaussNoise(var_limit=(10.0, 50.0), p=0.5),])  # Gaussian Noise 추가

    train_dataset = StratifiedImageDataset(train_fold_df, data_dir=args.train_data_dir, mode='train',min_polygon_bbox_size=args.min_polygon_bbox_size, max_polygon_bbox_size=args.max_polygon_bbox_size, pct=0.1, max_points=16,) #transforms=transform)
